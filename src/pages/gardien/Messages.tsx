...
const ROLE_ICON: Record<string, string> = {
  syndic: 'manage_accounts', resident: 'person', gardien: 'engineering',
}

export default function GardienMessages() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const { syndics = [], residents = [] } = contactsRes.data.data ?? {}
      setContacts([...syndics, ...residents])
          ...
          {newContacts.length > 0 && (
            <section className="space-y-2">
              ...
              {newContacts.map((c) => (
                <button
                  key={c._id}
                  onClick={() => navigate(`/gardien/chat/${c._id}`)}
            ...
            <section className="space-y-2">
              ...
              {conversations.map((conv) => {
                ...
                return (
                  <button
                    key={conv.partner.id}
                    onClick={() => navigate(`/gardien/chat/${conv.partner.id}`)}
                    className="w-full bg-surface-container-lowest rounded-2xl p-4 ambient-depth flex items-center gap-3 active:scale-[0.99] transition-transform text-left"
                  >
